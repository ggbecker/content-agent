...
class ContentSettings(BaseSettings):
    ...
    managed_path: Path = Field(
        default=Path.home() / ".cac-mcp" / "content",
    ...
    model_config = SettingsConfigDict(env_prefix="CAC_MCP_CONTENT__")
...
class HttpSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="CAC_MCP_SERVER__HTTP__")
...
class ServerSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="CAC_MCP_SERVER__")
...
class BuildSettings(BaseSettings):
    ...
    build_dir: Path = Field(
        default=Path.home() / ".cac-mcp" / "builds",
    ...
    model_config = SettingsConfigDict(env_prefix="CAC_MCP_BUILD__")
...
class TestingSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="CAC_MCP_TESTING__")
...
class JobsSettings(BaseSettings):
    ...
    database: Path = Field(
        default=Path.home() / ".cac-mcp" / "jobs.db",
    ...
    model_config = SettingsConfigDict(env_prefix="CAC_MCP_JOBS__")
...
class SecuritySettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="CAC_MCP_SECURITY__")
...
class LoggingSettings(BaseSettings):
    ...
    file: Optional[Path] = Field(
        default=Path.home() / ".cac-mcp" / "server.log",
        description="Log file path",
    )
    console: bool = Field(default=True, description="Enable console logging")

    model_config = SettingsConfigDict(env_prefix="CAC_MCP_LOGGING__")
...
class Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="CAC_MCP_", env_nested_delimiter="__"
    )
...
def _is_env_var_set(*keys: str) -> bool:
    ...
    # Main prefix is CAC_MCP_, nested delimiter is __
    if len(keys) == 1:
        env_var = f"CAC_MCP_{keys[0].upper()}"
    else:
        env_var = f"CAC_MCP_{'__'.join(k.upper() for k in keys)}"
